...
import { createSolaceConnectionUrl, createSolaceSessionClientName, decodeBinaryText, cloneSolaceMessage, createRequestId } from '../services/common-functions.service';
...
@Component({
  ...
  animations: [
    trigger('stateChange', [
      // state('pending', style({ weight: 'bold' })),
      state('failed', style({ color: 'red', weight: 'bold' })),
      state('waiting', style({ color: 'green', weight: 'bold' })),
      transition('* => waiting', [
...
export class ServerComponent implements OnInit {
  ...
  private _tracer = this.ots.getTracer(`${this.myConfig.runtimeConfig.serviceName}_SERVER`);

  constructor(
    public gameStatus: GameStatusService,
    private myApi: AccessApiService,
    private psp: SolaceMessageClient, private log: NGXLogger,
    private accessApiService: AccessApiService, private route: ActivatedRoute,
    private ots: OtelTraceService, private myConfig: LoadConfigService
    ) { }
  ...
  onService(): void {
    ...
    if (!this.isOnService) {
      const consumer$: Observable<MessageEnvelope> = this.psp.consume$({
        ...
        queueDescriptor: new QueueDescriptor({ type: QueueType.QUEUE, name: 'q-server' }),
      ...
      this._subscription = consumer$.pipe(
        ...
        next: (envelope: MessageEnvelope) => {
          ...
          this.log.debug("Received message: ", currRequest.id);
  ...
  onAckClick(reqId: string): void {
    ...
    this.log.debug(`Closing currSpan: ${traceId} and starting to process request: ${currRequest.id}...`);
    this._tracer.startActiveSpan(this.onAckClick.name, { kind: SpanKind.INTERNAL }, opentelemetry.trace.setSpan(context.active(), currSpan) , mySpan => {
      this.accessApiService.getRandomDataRemotely().pipe(takeUntil(this.destroy$)).subscribe({
        next: (data) => {
          this.log.debug(`Successfully processed request: ${currRequest.id}`);
          currRequest.receiverId = this.playerId;
          currRequest.receiverProcessId = createRequestId(currRequest.receiverId, this._seqNum);
          ...
          let mySDT = repMsg.getUserPropertyMap()!;
          // tracing
          ...
          this.psp.publish(`reply/server/${this._sessionConfig.clientName}`, repMsg);
        ...
        error: (error) => {
          // Dirty but at least work
          this.mRequests.get(currRequest.id)?.setStatus(RequestStatus.failed);
          this.log.error(error);
          mySpan.addEvent(RequestStatus[RequestStatus.failed]);
          mySpan.setStatus({ code: SpanStatusCode.ERROR });
          mySpan.end();
          currSpan.end();
        }
      });
